namespace Rokvi.IntegrationTest.Controllers;
...
public class CarsControllerTest : CustomWebApplicationFactory<Program>
{
        ...
        : base(testOutputHelper)
    {
        ...
        this.cultureInfo = new CultureInfo("is-IS");
    }
    ...
    public async Task GetPage_Invalid_Returns400BadRequestAsync(string direction)
    {
        ...
        var key = direction.ToLower(this.cultureInfo);
        var expectedValue = new Dictionary<string, string[]>
        {
            [key] = new string[] { $"'{this.FirstCharToUpper(direction)}' must be between 1 and 20. You entered 21." },
        };
        Assert.Equivalent(expectedValue, problemDetails.Errors);
    }
    ...
    public string FirstCharToUpper(string input) =>
        input switch
        {
            ...
            _ => string.Concat(input[0].ToString().ToUpper(this.cultureInfo), input.AsSpan(1))
        };
    ...
    public async Task PostCar_Invalid_Returns400BadRequestAsync()
    {
        ...
        Assert.Equivalent(
            ...
            {
                [nameof(SaveCar.Cylinders).ToLower(this.cultureInfo)] = new string[] { "'Cylinders' must be between 1 and 20. You entered 0." },
                [nameof(SaveCar.Make).ToLower(this.cultureInfo)] = new string[] { "'Make' must not be empty." },
                [nameof(SaveCar.Model).ToLower(this.cultureInfo)] = new string[] { "'Model' must not be empty." },
            },
    ...
    public async Task PutCar_Invalid_Returns400BadRequestAsync()
    {
        ...
        Assert.Equal(new string[] { "'Cylinders' must be between 1 and 20. You entered 0." }, problemDetails.Errors[nameof(SaveCar.Cylinders).ToLower(this.cultureInfo)]);
        Assert.Equal(new string[] { "'Make' must not be empty." }, problemDetails.Errors[nameof(SaveCar.Make).ToLower(this.cultureInfo)]);
        Assert.Equal(new string[] { "'Model' must not be empty." }, problemDetails.Errors[nameof(SaveCar.Model).ToLower(this.cultureInfo)]);
    }
